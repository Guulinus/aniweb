...
function AnimeDetailContent({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    setArcSeason(null);
    setArcSeasonResolved(seasons.length <= 1);
    ...
    fetch(`/api/tmdb/thumbnails?romaji=${encodeURIComponent(romaji)}${english ? `&english=${encodeURIComponent(english)}` : ''}&seasons=${seasonNumbers.join(',')}`)
      ...
      .then((data) => {
        ...
        // AniWorld groups multi-cour/multi-arc franchises (e.g. JoJo) into one page with
        // numbered seasons, but each AniList entry is titled after its own arc ("Stardust
        // Crusaders", "Diamond is Unbreakable") rather than "Season N" — match the arc name
        // against TMDB's season names (which follow the same convention) to find which
        // AniWorld season this specific entry actually corresponds to.
        if (Array.isArray(data.seasonNames) && data.seasonNames.length > 0) {
          const normalize = (s: string) => s.toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim();
          const animeTitles = [anime.title.english, anime.title.romaji]
            .filter(Boolean)
            .map((t) => normalize(t as string));
          const match = data.seasonNames.find(({ name }: { seasonNumber: number; name: string }) => {
            const n = normalize(name);
            return n.length > 3 && animeTitles.some((t) => t.includes(n));
          });
          if (match) setArcSeason(match.seasonNumber);
        }
        setArcSeasonResolved(true);
      ...
      .catch(() => setArcSeasonResolved(true));
  }, [anime, seasons]);
  ...
  const titleSeason = titleSeasonMatch ? parseInt(titleSeasonMatch[1]) : (arcSeason ?? null);
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10 -mt-40">
        <div className="flex flex-col md:flex-row gap-6 md:gap-8">
          {/* Poster — top-padded to match the info column's pt-2 md:pt-16, so the poster's
              top edge lines up with the format/episode/year badge row instead of the title. */}
        ...
        <section className="mt-12 mb-10 animate-fade-in stagger-3">
          ...
          {aniworldLoading || !arcSeasonResolved ? (
...
export default function AnimeDetailClient({ params }: { params: { slug: string } }) {
  return (
