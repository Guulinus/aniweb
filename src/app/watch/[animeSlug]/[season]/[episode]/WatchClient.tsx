...
function WatchContent({ animeSlug, season, episode }: { animeSlug: string; season: string; episode: string }) {
  ...
  const [episodeThumbnail, setEpisodeThumbnail] = useState<string>('');
  ...
  const episodeThumbnailRef = useRef(episodeThumbnail);
  ...
  }, [animeTitle, coverImage, episodeThumbnail]);

  // TMDB per-episode stills (preferred) and the AniWorld episode list (fallback) load
  // independently and after this episode's data is otherwise ready — recompute the
  // thumbnail for the currently displayed episode whenever either source updates.
  useEffect(() => {
    const fromTmdb = mergedThumbnails[episodeNum];
    const fromAniworld = episodeList.find(e => e.episode === episodeNum)?.thumbnail;
    setEpisodeThumbnail(fromTmdb || fromAniworld || '');
  }, [mergedThumbnails, episodeList, episodeNum]);

  // coverImage and episodeThumbnail arrive via fetches that are separate from (and often
  // slower than) the episode links that trigger the initial history/watchlist write, so
  // backfill them here once available in case that earlier write happened first without them.
  useEffect(() => {
    if ((!coverImage && !episodeThumbnail) || !animeId) return;
    try {
      const history = safeParseJSON(localStorage.getItem('watchHistory') ?? '[]', []);
      let changed = false;
      const patched = history.map((h: any) => {
        if (h.animeSlug === animeSlug && h.season === seasonNum && h.episode === episodeNum) {
          const next = { ...h };
          if (coverImage && !next.coverImage) { next.coverImage = coverImage; changed = true; }
          if (episodeThumbnail && !next.thumbnail) { next.thumbnail = episodeThumbnail; changed = true; }
          return next;
        }
        return h;
      });
      if (changed) localStorage.setItem('watchHistory', JSON.stringify(patched));
    } catch {}
    if (!coverImage) return;
    try {
      const watchlist = safeParseJSON(localStorage.getItem('watchlist') ?? '[]', []);
      let changed = false;
      const patched = watchlist.map((e: any) => {
        if (e.animeId === animeId && !e.coverImage) {
          changed = true;
          return { ...e, coverImage };
        }
        return e;
      });
      if (changed) localStorage.setItem('watchlist', JSON.stringify(patched));
    } catch {}
  }, [coverImage, episodeThumbnail, animeId, animeSlug, seasonNum, episodeNum]);
      ...
      fetch(`/api/aniworld/episode/${epId}`)
        ...
        .then((data) => {
          ...
          if (data.available && data.links?.length > 0) {
            ...
            const history = safeParseJSON(localStorage.getItem('watchHistory') ?? '[]', []);
            const entry = { animeSlug, animeId, season: seasonNum, episode: episodeNum, title: animeTitleRef.current, coverImage: coverImageRef.current, thumbnail: episodeThumbnailRef.current, watchedAt: Date.now() };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 pt-16 pb-32">
      ...
      {loading ? (
        ...
        <>
          <VideoPlayer links={links} episodeTitle={currentEpTitle || episodeTitle} animeId={animeId} idMal={idMal} episodeNum={episodeNum} seekTo={seekTo} skipData={skipData} onComplete={() => setCompleted(true)} nextEpisodeUrl={nextEpisodeLink} coverImage={coverImage} episodeThumbnail={episodeThumbnail} />
      ...
      {sortedSeasons.length > 0 && (
        <div className="mt-6">
          ...
          <div
            ...
            style={{ gridTemplateRows: episodeListOpen ? '1fr' : '0fr' }}
          >
...
export default function WatchClient({ params }: { params: { animeSlug: string; season: string; episode: string } }) {
  return (
